...
export function MonitorGrid({ cameras }: { cameras: Camera[] }) {
  const [layout, setLayout] = useState('3x3')
  return <section className="monitor-grid">
    <div className={`camera-grid camera-grid--${layout.replace('x', '-')}`}>
      {cameras.map((camera) => <CameraGridBox key={camera.id} box={{ id: String(camera.id), label: `CAM-${String(camera.id).padStart(2, '0')} ${camera.name}`, state: 'normal' }} />)}
    </div>
